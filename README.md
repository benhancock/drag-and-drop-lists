...
- Cycle task types from Obsidian's configurable mobile toolbar.
...
Touch interactions use an expanded marker hit area, a mobile drag threshold, and a preview positioned beneath the finger. Native text selection and workspace swipe gestures remain available when a list drag is not active.
...
The command cycles through these Markdown markers:
...
How alternate task markers look depends on the active theme or CSS snippets. The Markdown markers remain portable even when a theme displays them as ordinary checkboxes.
...
Drag and Drop Lists processes the active editor locally. It does not make network requests, collect analytics, access the clipboard, or read and write files through the vault API. Its only saved value is the cursor-placement preference.
